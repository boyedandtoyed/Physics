...
MUTATIONS = [
    ...
    ("kruskal-half", "the exponent carries the factor 1/2r_s", TD,
     "  const V = Math.exp((eddingtonFinkelsteinV(radius, startRadius) - boost) / (TWO * HORIZON));",
     "  const V = Math.exp((eddingtonFinkelsteinV(radius, startRadius) - boost) / HORIZON);"),
    ("kruskal-no-boost", "the plane is anchored at the horizon crossing, not at r0", TD,
     "  const boost = horizonCrossingV(startRadius);",
     "  const boost = 0;"),
    ("kruskal-boost-sign", "the anchor is subtracted from v and from u alike", TD,
     "  const U = Math.exp(-(eddingtonFinkelsteinU(radius, startRadius) - boost) / (TWO * HORIZON));",
     "  const U = Math.exp(-(eddingtonFinkelsteinU(radius, startRadius) + boost) / (TWO * HORIZON));"),
